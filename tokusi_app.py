...
with col1:
    with st.container(border=True):
        ...
        if st.button("移動する", key="guidance_home"):
            st.switch_page("pages/1_📚_指導支援内容.py")


    with st.container(border=True):
        st.markdown("#### 📈 分析方法")
        st.write("教育学や心理学に基づいた様々な分析方法の解説と、実践で使えるツールを提供します。")
        if st.button("移動する", key="analysis_home"):
            st.switch_page("pages/3_📈_分析方法.py")
...
with col2:
    with st.container(border=True):
        ...
        if st.button("移動する", key="chart_home"):
            st.switch_page("pages/2_📊_発達チャート作成.py")


    with st.container(border=True):
        st.markdown("#### 📝 フィードバック")
        st.write("アプリの改善や、新しい指導実践の共有など、皆様からのご意見をお待ちしています。")
        if st.button("移動する", key="feedback_home"):
            st.switch_page("pages/4_📝_フィードバック.py")
