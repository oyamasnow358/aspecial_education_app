...
"""
            ...
            if is_excel_mode:
                # --- Excelモード: 指示内容はそのまま、出力だけJSONにし、整形を指示 ---
...
【JSON出力フォーマット】
{{
  "needs": "1.特別な教育的ニーズの全文（上記の整形ルールに従うこと）",
  "accommodations": "2.合理的配慮の実施内容の全文（上記の整形ルールに従うこと）"
}}
"""
            else:
                # --- 通常モード ---
...
【JSON出力フォーマット】
{{
  "goals": "1.所属校の支援目標・機関名の全文（整形ルールに従う）",
  "support": "2.支援内容の全文（整形ルールに従う）"
}}
"""
            else:
                # --- 通常モード ---
...
【JSON出力フォーマット】
{{
  "policy": "1.指導方針の全文（整形ルールに従う）",
  "status_1": "① 健康の保持の内容（整形ルールに従う）",
  "status_2": "② 心理的な安定の内容",
  "status_3": "③ 人間関係の形成の内容",
  "status_4": "④ 環境の把握の内容",
  "status_5": "⑤ 身体の動きの内容",
