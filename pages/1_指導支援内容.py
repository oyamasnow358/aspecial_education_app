...
with cols[0]:
    selected_category = st.selectbox("１. カテゴリーを選択", list(guidance_data.keys()))

with cols[1]:
    if selected_category:
        selected_subcategory = st.selectbox("２. 該当する項目を選択", list(guidance_data[selected_category].keys()))
...
if st.button("適した指導・支援を表示", type="primary", use_container_width=True):
    if detail_data:
        st.markdown("---")
        st.subheader(f"📌 「{selected_detail}」に適した指導・支援")
