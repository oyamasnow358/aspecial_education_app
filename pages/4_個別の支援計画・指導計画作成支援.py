...
【出力項目】：
1.特別な教育的ニーズ
・入力から共通性を見出し、2～3つの抽象的カテゴリーに分類する（表記は①～③の形）。
・各ニーズは200〜300字程度で、児童・生徒の実態を柔らかく教育的にまとめる。

2.合理的配慮の実施内容
・上記①のニーズに対応した配慮を少なくとも2つ以上提案する。
・200〜300字程度で具体的に示す（例：視覚・触覚の活用、落ち着ける環境設定、視覚的支援など）。
...
【出力項目】：
1.所属校の支援目標（2～3つ）
・入力された各「特別な教育的ニーズ」に対応して、学校としての支援目標を1つずつ設定する。
・各目標は200字前後で記述し、柔らかく教育的な表現でまとめる。

2.各目標に連動した支援内容（2～3つ）
・上記の支援目標に対応する具体的な支援内容を提案する。
・支援内容も200字前後で、学校現場で実践可能な形に整える。


【条件】：
- 添付資料がある場合（「プランA」や「個別の教育支援計画」）にある書き方を参考にし、学校現場で活用できる具体性をもたせること。
- この後、この「所属校の支援目標」及び「各目標に連動した支援内容」を「特別な教育的ニーズ」⇒「所属校の支援目標」及び「各目標に連動した支援内容」（今回のプロンプトはここ）⇒「指導方針」（プランBや個別の指導計画）の順で具体化していくのでそのつもりで表現する。
- 「～です。～ます。」調ではなく「～である。」調で統一する。
- 語尾は断定的すぎず、支援的・協働的なニュアンスを含める。""", language="text")
...
- 「1.指導方針」は特別な教育的ニーズと所属校の支援目標の①～③に連動した形で
